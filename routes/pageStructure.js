...
router.get('/:page', optionalAuth, async (req, res, next) => {
  try {
    ...
    // Якщо помилка пов'язана з базою даних, повертаємо успішну відповідь з null
    const isDbError = error.name === 'SequelizeConnectionError' || 
    ...
    if (isDbError) {
      console.warn('⚠️  Database not available, returning empty structure:', error.message);
      return res.json({
        success: true,
    ...
    // Для інших помилок також повертаємо успішну відповідь з null, щоб frontend не падав
    console.warn('⚠️  Unexpected error, returning empty structure:', error.message);
    return res.json({
      success: true,
      page: req.params.page,
      structure: null
