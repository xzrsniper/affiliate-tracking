      ...
      <div className="space-y-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500">{t('admin.affiliatesPanelSubtitle')}</p>
            </div>
                  ...
                  <tr key={a.user_id} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-2 text-right">
                      <input
                        ...
                        min="0"
                        max="100"
                        value={edits[a.user_id]?.percent ?? ''}
                        onChange={(e) => setEdits((prev) => ({ ...prev, [a.user_id]: { ...(prev[a.user_id] || {}), percent: e.target.value } }))}
                        className="w-20 text-right px-2 py-1 border border-slate-200 rounded-md"
                      />
                    </td>
                    <td className="px-3 py-2 text-right">
                      <input
                        ...
                        step="0.01"
                        min="0"
                        value={edits[a.user_id]?.balance ?? ''}
                        onChange={(e) => setEdits((prev) => ({ ...prev, [a.user_id]: { ...(prev[a.user_id] || {}), balance: e.target.value } }))}
                        className="w-24 text-right px-2 py-1 border border-slate-200 rounded-md"
                      />
                    </td>
                    <td className="px-3 py-2 text-right">
                      <button
                        onClick={() => saveAffiliateSettings(a.user_id)}
                        disabled={updating}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white overflow-x-auto">
          <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <h2 className="font-bold text-slate-900">{t('admin.moderationTitleShort')}</h2>
                    ...
                    <td className="px-3 py-2 text-right">
                      {modStatus === 'pending' ? (
                        <div className="inline-flex gap-2">
                          <button onClick={() => handleModeration(item.id, 'approve')} disabled={updating} className="p-1.5 rounded bg-green-600 text-white">
                            <Check className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleModeration(item.id, 'reject')} disabled={updating} className="p-1.5 rounded bg-red-600 text-white">
                            <X className="w-4 h-4" />
                          </button>
                        </div>
                      ) : (
